...
async function loadTagConfig() {
    try {
        ...
        if (error.code === "ENOENT") {
            console.log(
                "Tag config file not found. Creating a new one with default categories."
            );
            tagConfig = {
                subject: [
                    {
                        ...
                        title: "Nature",
                        description:
                            "Images of natural landscapes, flora, or fauna"
                    },
                    {
                        name: "tech",
                        title: "Technology",
                        description:
                            "Images related to technological devices or concepts"
                    },
                    {
                        name: "people",
                        title: "People",
                        description:
                            "Images featuring individuals or groups of people"
                    }
                ],
                version: [
                    {
                        name: "1_0_0",
                        title: "Image set Version 1",
                        description: "Images from Version 1"
                    },
                    {
                        name: "1_1_0",
                        title: "Image set Version 1.1",
                        description: "Images from Version 1.1"
                    },
                    {
                        name: "2_0_0",
                        title: "Image set Version 2.0",
                        description: "Images from Version 2"
                    }
                ],
                general: [
                    {
                    ...
                    },
                    {
                        name: "colorful",
                        title: "Colorful",
                        description: "Images with a vibrant color palette"
                    },
                    {
                        name: "monochrome",
 ...
 */
function getAllTags() {
    return [...tagConfig.subject, ...tagConfig.version, ...tagConfig.general];
}
...
async function promptForNewTag() {
    console.log(
        "Name is the unique identifier for the tag, while Title is the user-friendly name."
    );
    const name = await input({
        message: "Enter the name for the new tag (or press Enter to cancel):"
    });
    if (!name) return null;

    const title = await input({
        message: "Enter a title for the new tag (press Enter to use the name):",
        default: name
    });

    const description = await input({
        message: "Enter a description for the new tag:"
    });
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                const existingMetadata = imageMetadata.find(
                    (img) => img.filename === relativePath
                );

                if (ext === ".webp") {
                    if (!existingMetadata) {
                        console.log(
                            `Adding metadata for existing WebP file: ${relativePath}`
                        );
                        updateImageMetadata(relativePath, [], "");
                    } else {
                        console.log(
                            `Skipping existing WebP file with metadata: ${relativePath}`
                        );
                    }
                } else if (
                    ...
                    console.log(
                        `Skipping: ${relativePath} (already processed or unsupported format)`
                    );
                }
            }
        }
...
async function main() {
    ...
    const needsMigration = imageMetadata.some(
        (img) => img.tags && img.tags.some((tag) => typeof tag === "object")
    );
    if (needsMigration) {
    ...
    while (true) {
        ...
        if (action === "tag") {
            ...
            for (const image of selectedImages) {
                const currentMetadata = imageMetadata.find(
                ...
                const label = await input({
                    message:
                        "Enter a label for the image (press Enter to keep current label):",
                    default: currentMetadata.label
                });

                const newTags = await promptForTags(
                    image,
        ...
        if (action === "view") {
            ...
            for (const image of imageMetadata) {
                ...
                for (const tagName of image.tags) {
                    ...
                    if (fullTag) {
                        console.log(
                            `    - ${fullTag.name}: ${fullTag.description}`
                        );
                    } else {
            ...
            while (true) {
                ...
                if (tagAction === "add") {
                    ...
                    if (newTag) {
                        ...
                        await saveTagConfig();
                        console.log(
                            `New tag "${newTag.name}" added successfully.`
                        );
                    }
                }
